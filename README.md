...
- `npm start`는 `package.json`에 지정된 `--host 0.0.0.0 --port 3000`으로 실행됩니다. HTTP GET/query 방식의 콜백만 지원하며 HTTPS 프록시/배포용 구성은 포함하지 않습니다.
- 토큰 요청은 15초 제한이며 HTTP 오류와 비 JSON 응답도 화면에 표시합니다.

주소와 포트는 `package.json`의 `scripts.start`에서 직접 변경할 수 있습니다.

```json
"start": "node server.js --host 0.0.0.0 --port 8080"
```

저장한 뒤 `npm start`를 실행하세요. 일회성으로 바꾸려면 `npm start -- --host 0.0.0.0 --port 8080`을 사용할 수도 있습니다. 뒤에 전달한 옵션이 우선합니다. CLI 옵션이 없는 경우에는 기존 `HOST`/`PORT` 환경변수를 사용하고, 둘 다 없으면 `127.0.0.1:3000`을 사용합니다.

### PC IP로 접속하기

기존 서버를 Ctrl+C로 종료한 후 PowerShell에서 실행하세요.

```powershell
npm start -- --host 0.0.0.0 --port 8080
```

`ipconfig`로 확인한 PC의 IPv4 주소가 `192.168.0.10`이면 `http://192.168.0.10:8080`으로 접속하고, 인증 제공자에는 `http://192.168.0.10:8080/callback`을 등록하세요. `0.0.0.0`은 바인딩 설정이며 브라우저 접속 주소가 아닙니다. 특정 IP에만 바인딩하려면 `--host`에 해당 IP를 직접 지정하세요.

`--host 0.0.0.0`은 PC 네트워크 인터페이스의 IP를 Host 검사에 자동으로 허용합니다. 다른 PC에서 접속하려면 해당 포트의 방화벽 인바운드 허용도 필요합니다. 설정 변경 후에는 서버를 재시작해야 합니다.
